	...
	public static void main(String args[]) throws IOException, InterruptedException, ProtocolException {
		...
		try {
			...
			try {
				...
				if (downloadURI == null && coverURI == null) {
					...
					if (uri.isTrackLink()) {
						...
						if (track == null) {
							throw new JustifyException("[ERROR] Track not found");
						}
						...
						if (playlist == null) {
							throw new JustifyException("[ERROR] Playlist not found");
						}
						...
						if (album == null) {
							throw new JustifyException("[ERROR] Album not found");
						}
						...
						throw new JustifyException("[ERROR] Track, album or playlist not specified");
					}
				} // Cover command
				else if (coverURI != null) {
					if (uri.isAlbumLink()) {
				...
				throw new JustifyException("[ERROR] Spotify URI is not valid");
			}

		} catch (JustifyException je) {
			...
			try {
				...
				System.err.println("[ERROR] Problem disconnecting");
			}
		}
	}
	...
	private void downloadTrack(Justify justify, Track track, String parent, String bitrate, String option, Integer index) throws JustifyException, TimeoutException, ProtocolException {
		...
		try {
			...
			if (option.equals("album")) {
				...
				// Numbering of filename when downloads a playlist. numbering = "index" : e.g. "01"
				filename = (index < 10 ? "0" : "") + index.toString() + " ";
			}
			...
			if (!file.exists()) {
				...
				if (option.equals("track") || option.equals("playlist")) {
					...
					System.out.print("[" + f.format((index - 1) * 100 / track.getAlbum().getTracks().size()) + "%] " + sanearNombre(filename));
				}
				...
				if (!clean) {
					try {
						...
						if (option.equals("playlist")) {
							comments.fields.add(new CommentField("TRACKNUMBER", index.toString()));
						} else {
